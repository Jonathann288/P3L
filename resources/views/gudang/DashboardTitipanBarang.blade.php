    ...
    <body class="font-sans bg-gray-100 text-gray-800 grid grid-cols-1 md:grid-cols-[250px_1fr] min-h-screen"
                                ...
                                <div x-show="open" x-transition.duration.300ms class="mt-4 text-sm space-y-2 border-t pt-4">
                                    <div class="grid grid-cols-1 gap-2">
                                        <div>
                                            ...
                                            <span class="text-gray-600">{{ $titipan->penitip ? $titipan->penitip->email_penitip : '-' }}</span>
                                        </div>
                                        
                                        <div>
                                            <strong class="text-gray-700">Tanggal Akhir Penitipan:</strong>
                                            <span class="text-gray-600">{{ $titipan->tanggal_akhir_penitipan ? $titipan->tanggal_akhir_penitipan->format('d M Y') : '-' }}</span>
                                        </div>
                                        
                                        <div>
                                            <strong class="text-gray-700">Tanggal Batas Pengambilan:</strong>
                                            <span class="text-gray-600">{{ $titipan->tanggal_batas_pengambilan ? $titipan->tanggal_batas_pengambilan->format('d M Y') : '-' }}</span>
                                        </div>
                                        
                                        <div>
                                            <strong class="text-gray-700">Tanggal Pengambilan Barang:</strong>
                                            <span class="text-gray-600">{{ $titipan->tanggal_pengambilan_barang ? $titipan->tanggal_pengambilan_barang->format('d M Y') : 'Belum diambil' }}</span>
                                        </div>
                                    </div>

                                    <!-- Foto Barang Section - DIPERBAIKI -->
                                    <div class="mt-4">
                                        <strong class="text-gray-700 block mb-2">Foto Barang:</strong>
                                        
                                        @php
                                            $fotoBarang = $titipan->foto_barang;
                                            // Debug: convert string JSON to array if needed
                                            if (is_string($fotoBarang)) {
                                                $fotoBarang = json_decode($fotoBarang, true) ?: [];
                                            }
                                        ...
                                        @if(!empty($fotoBarang) && count($fotoBarang) > 0)
                                                ...
                                                @foreach($fotoBarang as $index => $foto)
                                                    @if(!empty($foto))
                                                        <div class="text-center">
                                                            ...
                                                            <div class="text-xs text-red-500 mt-1 hidden">
                                                                Gagal memuat foto
                                                            </div>
                                                            
                                                            <!-- File info -->
                                                            <div class="text-xs text-gray-400 mt-1 truncate" title="{{ $foto }}">
                                                                {{ basename($foto) }}
                                                            </div>
                                                        </div>
                                                    @endif
                                            ...
                                            <!-- Alternative access methods for debugging -->
                                            <div class="mt-2 text-xs text-gray-400">
                                                <details class="cursor-pointer">
                                                    <summary>Debug Info (Klik untuk expand)</summary>
                                                    <div class="mt-2 p-2 bg-gray-50 rounded text-xs">
                                                        <div><strong>Raw Data:</strong> {{ json_encode($titipan->foto_barang) }}</div>
                                                        <div><strong>Processed Array:</strong> {{ json_encode($fotoBarang) }}</div>
                                                        <div><strong>Count:</strong> {{ count($fotoBarang) }}</div>
                                                        @foreach($fotoBarang as $idx => $f)
                                                            <div><strong>Path {{ $idx + 1 }}:</strong> {{ asset('storage/' . $f) }}</div>
                                                        @endforeach
                                                    </div>
                                                </details>
                                            </div>
                                        @else
                                            <div class="text-center py-4">
                                                <div class="w-24 h-24 mx-auto bg-gray-100 rounded border flex items-center justify-center">
                    ...
                    <form id="edit-form" method="POST" :action="getFormAction()" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <input type="email" 
                                    name="email_penitip" 
                                    x-model="formData.email_penitip" 
                                    class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                                    required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" 
                                    ...
                                    x-model="formData.tanggal_penitipan" 
                                    class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                                    required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" 
                                    ...
                                    x-model="formData.tanggal_akhir_penitipan" 
                                    class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" 
                                    name="tanggal_batas_pengambilan" 
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <p class="text-sm text-gray-500 mt-1">Dapat memilih beberapa foto sekaligus. Format: JPG, PNG, GIF. Maksimal 2MB per foto.</p>
                                
                                <!-- Preview container -->
                                <div id="image-preview" class="mt-3 grid grid-cols-2 md:grid-cols-3 gap-2" style="display: none;">
        ...
        <!-- Modal Image Viewer -->
        <div id="imageModal" class="fixed inset-0 bg-black bg-opacity-75 z-60 flex items-center justify-center p-4" style="display: none;">
